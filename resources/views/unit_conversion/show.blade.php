...
@section('title', 'Modifica Conversione')
...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route('unit-conversions.update', [$unitConversion->id]) }}">
            @csrf
            @method('PUT')
            @if ($errors->any())
                    ...
                    <div class="input-group mb-3">
                        ...
                        <select id="from_unit_of_measure_id" name="from_unit_of_measure_id" class="form-control" required>
                            ...
                            @foreach($unitOfMeasures as $uom)
                                <option value="{{ $uom->id }}" {{ $unitConversion->from_unit_of_measure_id == $uom->id ? 'selected' : '' }}>
                                    {{ $uom->name }} ({{ $uom->symbol }})
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="col-6">
                    ...
                    <div class="input-group mb-3">
                        ...
                        <input type="text" id="from_quantity" name="from_quantity"
                            value="{{ number_format($unitConversion->from_quantity, 2, ',', '.') }}"
                ...
                <div class="col-6">
                    ...
                    <div class="input-group mb-3">
                        ...
                        <select id="to_unit_of_measure_id" name="to_unit_of_measure_id" class="form-control" required>
                            <option value="">-- Seleziona --</option>
                            @foreach($unitOfMeasures as $uom)
                                <option value="{{ $uom->id }}" {{ $unitConversion->to_unit_of_measure_id == $uom->id ? 'selected' : '' }}>
                                    {{ $uom->name }} ({{ $uom->symbol }})
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="col-6">
                    ...
                    <div class="input-group mb-3">
                        ...
                        <input type="text" id="to_quantity" name="to_quantity"
                            value="{{ number_format($unitConversion->to_quantity, 2, ',', '.') }}"
                            class="form-control" placeholder="0,00" autocomplete="off" required>
                    </div>
                </div>
